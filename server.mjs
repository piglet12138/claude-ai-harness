...
const agenticSystemPromptText = [
  ...
  "**图标 = 专业感主要来源（强烈推荐）**：环境已装 react-icons + react + react-dom + sharp。把图标渲染成 PNG 内嵌（PowerPoint 兼容性最好）。**run_code 是 .cjs 不支持顶层 await，整段必须包在 (async()=>{ ... })() 里。** 骨架：",
  "```javascript",
  'const pptxgen=require("pptxgenjs"), React=require("react"), RD=require("react-dom/server"), sharp=require("sharp"), fa=require("react-icons/fa");',
  "(async () => {",
  "  const NAVY='08263F',TEAL='0E9488',CYAN='22D3EE',INK='0E2436',MUTED='5E7A8C',LIGHT='F2F8FA',WHITE='FFFFFF';",
  ...
  "  // 几何自检卡口：把每个元素的 opts 用 track() 包一层（记录边界），写文件前断言全在画布内",
  "  const els=[]; const track=o=>{els.push({x:o.x,y:o.y,w:o.w,h:o.h}); return o;};",
  "  const p=new pptxgen(); p.layout='LAYOUT_WIDE';  // 画布 13.33 x 7.5 英寸",
  "  const radar=await icon(fa.FaSatelliteDish, WHITE);  // react-icons 颜色带 #；pptxgenjs 颜色不带 #",
  "  const s=p.addSlide(); s.background={color:LIGHT};",
  "  // 页眉：英文 kicker + 衬线大标题（纯文字标题可不 track）",
  "  s.addText('CORE STRENGTHS',{x:0.7,y:0.45,w:9,h:0.32,fontFace:BODY,fontSize:12,bold:true,color:TEAL,charSpacing:3});",
  "  s.addText('六大核心优势',{x:0.7,y:0.74,w:11.4,h:0.7,fontFace:HEAD,fontSize:30,bold:true,color:INK});",
  "  // 圆形图标徽章 + 卡片（容器类元素都用 track() 包 opts）",
  "  s.addShape(p.shapes.ROUNDED_RECTANGLE, track({x:0.7,y:2,w:3.9,h:2.2,fill:{color:WHITE},rectRadius:0.1,shadow:shadow()}));",
  "  s.addShape(p.shapes.OVAL, track({x:1.0,y:2.3,w:0.9,h:0.9,fill:{color:TEAL},shadow:shadow()}));",
  "  s.addImage(track({data:radar,x:1.23,y:2.53,w:0.44,h:0.44}));",
  "  // 写文件前确定性自检：任何元素越界就抛错（不写文件）→ 你会收到报错、改坐标后重跑",
  "  const bad=els.filter(e=>e.x<0||e.y<0||e.x+e.w>13.13||e.y+e.h>7.3);",
  "  if(bad.length) throw new Error('PPTX 越界元素，必须修正坐标后重跑：'+JSON.stringify(bad));",
  ...
  "**安全红线**：① 画布 13.33×7.5，**必须用上面的 track()+writeFile 前断言卡口**（容器/卡片/图片/色条都 track），越界会 throw 不写文件——你据报错改坐标重跑，直到全部 x+w ≤ 13.13 且 y+h ≤ 7.3（无预览、越界会被裁且不报错，这个卡口是唯一可靠防线）；② shadow 用工厂函数、勿复用同一对象；③ ShapeType.line 的 w/h 不能为负（朝左/上用 beginArrowType:'triangle'）；④ 颜色十六进制不带 #；⑤ 元素间留 ≥0.2 间距、不重叠；多页 deck 每页都要 track 自检。",
  "",
