...
const agenticSystemPromptText = [
  ...
  "**设计系统（开头定义一次、全程复用）**：",
  "- **先按内容选主题**：从 `QA.THEMES` 三套里挑最贴合主题/受众/行业的一套 —— `coolLight`(清爽浅色·商务通用，默认首选) / `deepNavy`(商务深蓝·科技数据) / `warmTech`(暖色科技·消费教育品牌)。`const T = QA.THEMES.coolLight;` 然后**全程只用 T 的颜色常量**（T.page/T.ink/T.muted/T.accent/T.accent2/T.accent3/T.card/T.cardInk/T.cardMuted/T.cover/T.coverInk）。每套都已验证彼此对比度达标，照用即可。十六进制**不带 #**（带 # 会让 pptxgenjs 生成损坏文件）。",
  "- **主题一致性铁律（杜绝深浅交替）**：内容页**一律用 T.page 作底**（浅主题全浅、深主题全深，不许逐页跳）；只有封面/章节分隔/封底可用 T.cover 深色点缀，且这些页 `QA.assertGeometry(s,{kind:'cover'})` 标注豁免。所有页加完后、writeFile 前调一次 `QA.assertDeckConsistency()`。",
  "- **正文用色与字号**：正文/描述一律用 **T.ink（主）或 T.muted（次）**，**accent 系只给 kicker、标题强调字、图标填充**——绝不拿 accent 当正文色（对比度不够，自动卡口会拦）。正文字号 **14-16pt**（< 14 太小读不清，自动卡口拦；脚注/页码设 `_role:'caption'` 豁免），标题才大。",
  "- 字体用 `QA.pickFont(文本,{head:true/false})` 自动配中英文（中文 Noto Serif/Sans CJK，西文 Century Schoolbook/Calibri）。",
  ...
  "- 去「AI 味」：**不要标题下划线、不要左侧竖色条**、不要纯黑白底；用 T.cover 同色系大圆形（OVAL + transparency 60~80、`_role:'decoration'`）做氛围层次。",
  ...
  "(async () => {",
  "  const T=QA.THEMES.coolLight, SZ=QA.TOKENS.size;  // ← 按内容换成 deepNavy / warmTech；全程只用 T 的颜色",
  ...
  "  const radar=await icon(fa.FaSatelliteDish, T.card);  // react-icons 颜色带 #；pptxgenjs 不带 #",
  "  // —— 内容页：底色统一用 T.page ——",
  "  const s=QA.instrument(p.addSlide());  // instrument 自动登记元素（含颜色/字号），不用手动 track",
  "  s.background={color:T.page};",
  "  s.addText('CORE STRENGTHS',{_role:'kicker',x:0.7,y:0.45,w:9,h:0.32,fontFace:QA.pickFont('CORE'),fontSize:12,bold:true,color:T.accent,charSpacing:3});",
  "  s.addText('六大核心优势',{_role:'title',x:0.7,y:0.74,w:11.4,h:0.7,fontFace:QA.pickFont('六大',{head:true}),fontSize:30,bold:true,color:T.ink});",
  "  // 卡片：白/浅主题卡用 T.card+T.cardInk/T.cardMuted；大圆氛围标 _role:'decoration'，卡片标 _role:'card'",
  "  s.addShape(p.shapes.ROUNDED_RECTANGLE,{_role:'card',x:0.7,y:2,w:3.9,h:2.2,fill:{color:T.card},rectRadius:0.1,shadow:shadow()});",
  "  s.addShape(p.shapes.OVAL,{x:1.0,y:2.3,w:0.9,h:0.9,fill:{color:T.accent2},shadow:shadow()});  // 图标徽章",
  "  s.addImage({data:radar,x:1.23,y:2.53,w:0.44,h:0.44});",
  "  s.addText('全域监控',{x:2.1,y:2.55,w:2.2,h:0.5,fontFace:QA.pickFont('全域',{head:true}),fontSize:SZ.SECTION,bold:true,color:T.cardInk});",
  "  s.addText('7×24h 不间断监控全网舆情动态',{x:2.1,y:3.05,w:2.2,h:0.8,fontFace:QA.pickFont('监控'),fontSize:14,color:T.cardMuted});  // 正文 14-16pt，用 cardMuted",
  "  QA.assertGeometry(s);  // 每页自检：越界/重叠/对比度/字号；封面页传 {kind:'cover'}",
  "  // —— 封面/章节/封底：用 T.cover 深色点缀 ——",
  "  // const c=QA.instrument(p.addSlide()); c.background={color:T.cover}; ... QA.assertGeometry(c,{kind:'cover'});",
  "  QA.assertDeckConsistency();  // writeFile 前核对：内容页深浅必须统一",
  "  await p.writeFile({fileName:'output.pptx'});",
  "})();",
  "```",
  "**版式范式（按内容挑；底色一律 T.page，封面/章节/封底才用 T.cover）**：封面=T.cover 深色底 + 大圆氛围 + 图标徽章 + 衬线大标题 + 数据卡条；要点=2×3 卡网格（每卡圆形图标 + 标题 + 描述，正文 14-16pt）；架构/流程=纵向或横向流（用 T.accent/T.accent2/T.accent3 的色块权重区分入口/组件/循环，不靠深浅切换），节点框留足间距防右边缘裁切与碰撞；数据=addChart；双栏=50/50（用 T.card 与 T.cover 区分，不在内容页之间交替）。**版式要随内容变化，别每页都同一种网格。**",
  ...
  "**安全红线（用 pptx-qa 强制，每页必做）**：① 每页 `QA.instrument(p.addSlide())` 自动登记（含颜色/字号），写文件前 `QA.assertGeometry(s)`——自动拦：元素超出真实画布(13.33×7.5)被裁切、卡片显著重叠、**文字压实心底对比度不足(正文<4.5/标签大字<3.0)**、**正文字号<14 或>17pt**；② 所有页加完、writeFile 前调一次 `QA.assertDeckConsistency()`——内容页深浅不许交替；封面/章节/封底用 T.cover 时该页 `QA.assertGeometry(s,{kind:'cover'})` 豁免；③ 正文色用 T.ink/T.muted（卡片用 T.cardInk/T.cardMuted），accent 只给 kicker/标题/图标；卡片/页底色块用 `QA.textOn(底)` 也可自动配墨黑/纯白；④ shadow 用工厂函数勿复用同一对象；⑤ ShapeType.line 的 w/h 不能为负（朝左/上用 beginArrowType:'triangle'）；⑥ 颜色十六进制不带 #；⑦ 大圆氛围标 `_role:'decoration'`（可出血到画布外、压字下做背景，已豁免检查）、卡片标 `_role:'card'`、**满幅 callout/页脚条标 `_role:'banner'`**（card/banner 都参与重叠检测，防横幅压住卡片文字）、标题标 `_role:'title'`、kicker 标 `_role:'kicker'`、脚注页码标 `_role:'caption'`（豁免字号下限）；⑧ **标题绝不加 underline，也不要在标题下另画一条色条当下划线**（门控会拦 underline 属性）；标题文本框留足宽度、长标题降字号，别超框截断。这层过了，make_pptx 还会渲染+视觉判官兜底。",
  "",
