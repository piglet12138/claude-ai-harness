...
const agenticSystemPromptText = [
  ...
  "**安全红线（用 pptx-qa 强制，每页必做）**：① 每页 `QA.instrument(p.addSlide())` 自动登记（含颜色/字号），写文件前 `QA.assertGeometry(s)`——自动拦：元素超出真实画布(13.33×7.5)被裁切、卡片显著重叠、**文字压实心底对比度不足(正文<4.5/标签大字<3.0)**、**正文字号<14 或>17pt**；② 所有页加完、writeFile 前调一次 `QA.assertDeckConsistency()`——内容页深浅不许交替；封面/章节/封底用 T.cover 时该页 `QA.assertGeometry(s,{kind:'cover'})` 豁免；③ 正文色用 T.ink/T.muted（卡片用 T.cardInk/T.cardMuted），accent 只给 kicker/标题/图标；卡片/页底色块用 `QA.textOn(底)` 也可自动配墨黑/纯白；④ shadow 用工厂函数勿复用同一对象；⑤ ShapeType.line 的 w/h 不能为负（朝左/上用 beginArrowType:'triangle'）；⑥ 颜色十六进制不带 #；⑦ 大圆氛围标 `_role:'decoration'`（可出血到画布外、压字下做背景，已豁免检查）、卡片标 `_role:'card'`（参与重叠检测）、标题标 `_role:'title'`、kicker 标 `_role:'kicker'`、脚注页码标 `_role:'caption'`（豁免字号下限）。这层过了，make_pptx 还会渲染+视觉判官兜底。",
  "",
