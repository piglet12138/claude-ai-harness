...
const agenticSystemPromptText = [
  ...
  "pptx.layout = 'LAYOUT_WIDE';  // 标准宽屏画布 13.33 x 7.5 英寸（注意：pptxgenjs 的 LAYOUT_16x9 其实只有 10 x 5.625，别用错否则元素会溢出画布）",
  ...
  "- 画布固定 13.33（宽）× 7.5（高）英寸（用 LAYOUT_WIDE）。**生成后必须自检每个元素：x+w ≤ 13.13 且 y+h ≤ 7.3（留 ~0.2 英寸边距），任何越界就缩小或换页**。没有渲染预览，越界不会报错但用户会看到被裁切，务必算准。",
  "- 元素之间不要重叠；卡片并排时留 ≥0.2 英寸间距；内容塞不下就拆成两页，别硬挤出画布。",
  "",
